...
# law.go.kr는 법령 본문을 자바스크립트로 비동기 로딩한다 — 단순 HTTP GET으로 받은 HTML에는
# 메뉴/틀만 있고 실제 조문 텍스트가 없다. 그래서 헤드리스 브라우저로 페이지를 실제로 띄워서
# 자바스크립트가 본문을 채운 뒤의 최종 HTML을 캡처한다.
# 일반적인 스크립트 User-Agent는 차단될 수 있어 일반 브라우저처럼 보이는 값을 쓴다. 그래도
# 차단될 수 있으니 첫 실행은 반드시 수동으로 트리거해서 실제로 가져와지는지 확인할 것 (fetcher/README.md).
USER_AGENT = (
...
def fetch_and_store(key: str, source: dict, page) -> dict:
    url = source["url"]
    display_name = source.get("display_name", key)

    page.goto(url, wait_until="networkidle", timeout=45000)
    # networkidle 이후에도 일부 스크립트가 지연 렌더링하는 경우를 대비한 여유 대기.
...
def run():
    ...
    with sync_playwright() as p:
        ...
        for key, source in LAW_SOURCES.items():
            try:
                results.append(fetch_and_store(key, source, page))
